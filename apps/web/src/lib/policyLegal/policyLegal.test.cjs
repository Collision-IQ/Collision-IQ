...
const {
  buildRegulationVersionSnapshot,
  compareRegulationVersion,
  selectRegulationVersionForClaimReplay,
} = require("./regulationVersions.ts");
...
run("valid verified regulation seed record is accepted", () => {
  const normalized = normalizeVerifiedRegulationSeedRecord({
    ...
    source_publication_date: "2026-01-20T00:00:00.000Z",
    citation_source: "Official bulletin",
  ...
  assert.ok(normalized.supersededDate instanceof Date);
  assert.ok(normalized.sourcePublicationDate instanceof Date);
  assert.equal(normalized.citationSource, "Official bulletin");
  assert.equal(normalized.verifiedBy, "Policy Team");
  assert.equal(normalized.notes, "Source reviewed.");
});

run("regulation version hash is stable and excludes retrieval timestamp", () => {
  const first = buildRegulationVersionSnapshot({
    regulationId: "fl-parts_usage-fs-501-33",
    jurisdiction: "fl",
    effectiveDate: "2026-01-01",
    sourcePublicationDate: "2025-12-15",
    verificationStatus: "verified",
    citationSource: "Florida Senate",
    retrievalTimestamp: "2026-05-05T00:00:00.000Z",
    ruleText: "Aftermarket parts disclosure required.",
    citationText: "Fla. Stat. 501.33",
    sourceUrl: "https://example.test/fl",
  });
  const second = buildRegulationVersionSnapshot({
    regulationId: "fl-parts_usage-fs-501-33",
    jurisdiction: "FL",
    effectiveDate: "2026-01-01T00:00:00.000Z",
    sourcePublicationDate: "2025-12-15T00:00:00.000Z",
    verificationStatus: "verified",
    citationSource: "Florida Senate",
    retrievalTimestamp: "2026-05-06T00:00:00.000Z",
    ruleText: "Aftermarket parts disclosure required.",
    citationText: "Fla. Stat. 501.33",
    sourceUrl: "https://example.test/fl",
  });

  assert.equal(first.jurisdiction, "FL");
  assert.equal(first.version_hash, second.version_hash);
  assert.equal(first.retrieval_timestamp, "2026-05-05T00:00:00.000Z");
  assert.equal(second.retrieval_timestamp, "2026-05-06T00:00:00.000Z");
});

run("regulation version comparison detects changed source snapshots", () => {
  const previous = buildRegulationVersionSnapshot({
    regulationId: "pa-parts",
    jurisdiction: "PA",
    effectiveDate: "2026-01-01",
    verificationStatus: "verified",
    citationSource: "PA Code",
    retrievalTimestamp: "2026-05-05T00:00:00.000Z",
    ruleText: "Original rule text.",
    citationText: "31 Pa. Code 62.3",
  });
  const comparison = compareRegulationVersion(
    {
      regulationId: "pa-parts",
      jurisdiction: "PA",
      effectiveDate: "2026-01-01",
    ...
    {
      regulationId: previous.regulation_id,
  ...
  assert.deepEqual(comparison.changed_fields, ["ruleText"]);
  assert.equal(comparison.replay_safe, true);
});

run("point-in-time regulation replay selects historical version", () => {
  const versions = [
    {
      effectiveDate: "2026-01-01T00:00:00.000Z",
      supersededDate: "2026-06-01T00:00:00.000Z",
      versionHash: "old",
    },
    {
  ...
  assert.equal(
    selectRegulationVersionForClaimReplay(versions, "2026-03-15T00:00:00.000Z").versionHash,
    "old"
  );
  assert.equal(
    selectRegulationVersionForClaimReplay(versions, "2026-07-15T00:00:00.000Z").versionHash,
    "new"
  );
});
