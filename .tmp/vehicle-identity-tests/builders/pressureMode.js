...
exports.computeItemPressureMode = computeItemPressureMode;
exports.computeModelPressureMode = computeModelPressureMode;
exports.buildRebuttalOpeningLine = buildRebuttalOpeningLine;
exports.buildRebuttalClosingCta = buildRebuttalClosingCta;
exports.formatPressureMode = formatPressureMode;
// ─── Thresholds ────────────────────────────────────────────────────────────
 ...
 * | *             | < MEDIUM          | cautious      |
 */
function computeItemPressureMode(evidenceLevel, leverageScore) {
    const isWeak = evidenceLevel === "inferred" ||
        evidenceLevel === "unsupported";
    if (isWeak)
        return "cautious";
    if (leverageScore < MEDIUM_LEVERAGE_MIN)
        return "cautious";
    const isHigh = leverageScore >= HIGH_LEVERAGE;
    if (isHigh) {
        // incomplete proof + high leverage = carrier is clearly holding out
        if (evidenceLevel === "missing")
            return "assertive";
        // well-documented + high leverage = strongest ground
        if (evidenceLevel === "documented" || evidenceLevel === "referenced")
            return "assertive";
    }
 ...
 */
function computeModelPressureMode(params) {
    const topFindings = params.findingReasoning.slice(0, 5);
    const breakdown = topFindings.map((finding) => ({
        issue: finding.issue,
    ...
    if (breakdown.length < 2 && params.supplementItems.length > 0) {
        for (const item of params.supplementItems.slice(0, 3)) {
            breakdown.push({
                ...
                mode: computeItemPressureMode(mapSupplementKindToEvidenceHint(item.kind), item.leverageScore ?? 0),
            });
        }
    }
 ...
 */
function buildRebuttalOpeningLine(mode, subjectVehicle, repairPosition) {
    const position = lowercaseFirst(repairPosition);
    switch (mode) {
        case "assertive":
            return (`After a detailed review of the current estimate and supporting file for the ${subjectVehicle}, ` +
        ...
        case "explanatory":
            return (`After reviewing the current estimate for the ${subjectVehicle}, our position is that ${position} ` +
        ...
        case "cautious":
            return (`Based on the current file review for the ${subjectVehicle}, we have noted several items for consideration. ` +
 ...
 */
function buildRebuttalClosingCta(mode) {
    switch (mode) {
        case "assertive":
 ...
 */
function formatPressureMode(mode) {
    switch (mode) {
        case "assertive":
            return "Assertive";
        case "explanatory":
...
// ─── Internal helpers ──────────────────────────────────────────────────────
function buildModeRationale(mode, breakdown) {
    const top = breakdown[0];
    if (!top)
        return "Insufficient findings to determine dispute tone.";
    switch (mode) {
        case "assertive":
            return (`Top findings show ${top.evidenceLevel === "missing" ? "support not yet located in reviewed files" : "strong evidence"} ` +
                `with a leverage score of ${top.leverageScore}. The file supports a direct revision request.`);
        case "explanatory":
            return (`Top findings show ${top.evidenceLevel} evidence with a leverage score of ${top.leverageScore}. ` +
                `A clear explanation of the file gap is the appropriate approach.`);
        case "cautious":
            return (`Evidence is ${top.evidenceLevel} and leverage is low (${top.leverageScore}). ` +
                `A cautious, documentation-requesting stance is most defensible at this stage.`);
    }
}
function mapSupplementKindToEvidenceHint(kind) {
    switch (kind) {
