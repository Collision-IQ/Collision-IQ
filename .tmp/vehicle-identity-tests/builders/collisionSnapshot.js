...
const customerFacingText_1 = require("@/lib/ai/customerFacingText");
...
function buildCollisionSnapshot(input) {
    ...
    const snapshot = {
        ...
        evidenceCompleteness: {
            ...
            indexedFileCount: snapshotSafeReport.confidenceIntegrity.indexedFileCount ??
            ...
            reviewableFileCount: snapshotSafeReport.confidenceIntegrity.reviewableFileCount ??
            ...
            totalKnownFileCount: snapshotSafeReport.confidenceIntegrity.totalKnownFileCount ??
                snapshotSafeReport.confidenceIntegrity.uploadedFileCount,
...
function buildRepairPlanVerdict(renderModel) {
    ...
    const moreCompletePlan = posture.selectedEstimateLabel === "shop"
        ? "SHOP"
        : (0, estimatePosture_1.isCarrierSelectedPosture)(posture)
            ? "CARRIER"
            : "INCONCLUSIVE";
    ...
    const hasEstablishedMissingScope = renderModel.confidenceIntegrity.missingCriticalEvidence.length > 0 ||
        renderModel.supplementItems.some((item) => item.kind === "missing_verification");
    const carrierPlanStatus = moreCompletePlan === "SHOP" && hasEstablishedMissingScope
    ...
    const reason = incomplete
        ...
        : reasonBase;
    const carrierPlanDescriptor = buildCarrierPlanDescriptor(renderModel, carrierPlanStatus, moreCompletePlan);
...
function buildCarrierPlanDescriptor(renderModel, carrierPlanStatus, moreCompletePlan) {
    ...
    const scopeText = [
        ...renderModel.supplementItems.map((item) => `${item.category} ${item.title} ${item.rationale ?? ""} ${item.evidence ?? ""}`),
        ...renderModel.findingReasoning.map((finding) => `${finding.issue} ${finding.what_proves_it}`),
    ]
        .join(" ")
        .toLowerCase();
...
function buildVerdictLine(renderModel, repairPlanVerdict, estimateComparisons) {
    ...
    if (repairPlanVerdict.moreCompletePlan === "SHOP" &&
        hasLaborDelta &&
...
function buildTopDisputeItems(renderModel) {
    ...
    return displayFindings.slice(0, 3).map((finding) => ({
        ...
        nextAction: cleanSnapshotText(finding.next_action) ||
...
function rankSnapshotFindingsForDisplay(renderModel, rankedFindings) {
    if (!hasSideImpactRepairContext(renderModel))
...
function hasSideImpactRepairContext(renderModel) {
    const context = [
        ...renderModel.reportFields.documentedHighlights,
        ...renderModel.reportFields.documentedProcedures,
        ...renderModel.supplementItems.map((item) => `${item.category} ${item.evidence} ${item.rationale}`),
        ...renderModel.findingReasoning.map((finding) => `${finding.issue} ${finding.what_proves_it} ${finding.why_it_matters} ${finding.next_action}`),
    ].join(" ");
    return /\b(?:left|lt|lh|side|door|quarter|rocker|pillar|aperture|wheel|blind spot|side radar)\b/i.test(context);
}
function isGenericFrontEndFinding(finding) {
    const text = `${finding.issue} ${finding.what_proves_it} ${finding.why_it_matters} ${finding.next_action}`;
    return (/\b(?:front[-\s]?end|front structure|front support|tie bar|upper rail|front bumper|grille|hood|headlamp|front camera|front radar|radiator support|core support)\b/i.test(text) &&
        !/\b(?:side|left|lt|lh|door|quarter|rocker|pillar|aperture|wheel|alignment|blind spot|side radar)\b/i.test(text));
}
function snapshotDisplayScore(finding) {
    const text = `${finding.issue} ${finding.what_proves_it} ${finding.why_it_matters} ${finding.next_action}`;
    const base = finding.leverageScore ?? 0;
    const sideDriverBoost = /\b(?:side structure|left|door|quarter|rocker|pillar|aperture|wheel|adas|calibration|scan|repair completeness|complete repairs|fit)\b/i.test(text)
        ? 50
        : 0;
...
function sanitizeSnapshot(snapshot) {
    ...
    if (sanitized.valuationSnapshot.available &&
        !sanitized.valuationSnapshot.acvPreviewRange &&
        !sanitized.valuationSnapshot.dvPreviewRange) {
        const disclosure = sanitized.valuationSnapshot.disclosure &&
...
function cleanSnapshotText(value) {
    if (!value)
        return null;
    let output = (0, customerFacingText_1.toCustomerFacingText)(value)
        .replace(/\s+/g, " ")
    ...
    if (looksMalformedParserFragment(output))
        return null;
    output = output.replace(/\s{2,}/g, " ").trim();
    return output || null;
}
function formatComparisonDelta(row) {
    const label = cleanSnapshotText((0, presentationText_1.cleanOperationDisplayText)(row.operation)) ||
        cleanSnapshotText((0, presentationText_1.cleanOperationDisplayText)(row.partName)) ||
        cleanSnapshotText((0, presentationText_1.cleanOperationDisplayText)(row.category)) ||
        "Estimate line";
...
function looksMalformedParserFragment(value) {
    return (/\bProc\s*\d+\s*#?\*+/i.test(value) ||
