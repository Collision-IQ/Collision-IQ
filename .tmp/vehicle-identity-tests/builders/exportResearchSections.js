...
exports.buildExportResearchSections = buildExportResearchSections;
...
const INTERNAL_ONLY_CATEGORIES = ["Unsupported / Needs Review"];
function buildExportResearchSections(snapshot, options = {}) {
    if (!snapshot) {
        return [];
    }
    return [
        ...(options.includeInternalAudit
            ...
            : []),
        ...[...SUPPORT_CATEGORIES, ...(options.includeInternalAudit ? INTERNAL_ONLY_CATEGORIES : [])].flatMap((category) => {
...
function resolveCategoryTitle(snapshot, category) {
    ...
    const lawSources = snapshot.sourcesAccepted.filter((source) => source.supportCategory === "Verified Law" && hasVerifiedLawJurisdiction(source.jurisdiction));
    const hasJurisdictionRelevance = lawSources.some((source) => Boolean(source.jurisdiction?.trim()));
    return hasJurisdictionRelevance ? "Verified Law" : "Legal Support - Jurisdiction Not Established";
}
function formatCategoryBullets(snapshot, category) {
...
function hasNoOemSources(snapshot) {
    return !snapshot.sourcesAccepted.some((source) => source.supportCategory === "Verified OEM / Position Statement Support" ||
        (source.sourceType === "oem" && source.supportCategory === "General Research Leads - Not Make-Specific"));
}
function hasVerifiedLawJurisdiction(value) {
    const text = value?.trim();
    if (!text)
        return false;
    return !/^(not established|unknown|false|null|n\/?a)$/i.test(text);
}
function formatMeaningfulReference(value) {
    const text = value?.replace(/\s+/g, " ").trim();
    if (!text || /^source link$/i.test(text) || /^n\/?a$/i.test(text))
        return null;
    return text;
}
