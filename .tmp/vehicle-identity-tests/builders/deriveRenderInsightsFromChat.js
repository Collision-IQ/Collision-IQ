...
exports.deriveRenderInsightsFromChat = deriveRenderInsightsFromChat;
...
function deriveRenderInsightsFromChat(assistantAnalysis, vehicleApplicability) {
    const text = (assistantAnalysis ?? "").replace(/\r/g, "").trim();
    const narrative = (0, vehicleApplicability_1.sanitizeVehicleSpecificText)(extractNarrative(text), vehicleApplicability) || undefined;
    const request = (0, vehicleApplicability_1.sanitizeVehicleSpecificText)(extractRequest(text), vehicleApplicability) || undefined;
    return {
        narrative,
...
function looksLikeValuationOnlyNarrative(paragraph) {
    const lower = paragraph.toLowerCase();
    const hasValuationSignal = /\b(?:acv|actual cash value|market value|valuation|vehicle value|book value)\b/i.test(paragraph);
    const hasCurrency = /\$\s?\d[\d,]*(?:\.\d{2})?/.test(paragraph);
    const hasRepairSignal = /\b(?:repair|estimate|supplement|procedure|calibration|scan|blend|refinish|replace|labor|oem|operation|documentation|teardown)\b/i.test(paragraph);
    return hasValuationSignal && hasCurrency && !hasRepairSignal && lower.length < 240;
}
function extractSupplementItems(text, vehicleApplicability) {
    if (!text)
        return [];
    ...
    const candidates = blocks
        ...
        .filter((line) => (0, vehicleApplicability_1.isVehicleContentApplicable)(line, vehicleApplicability))
    ...
    for (const item of candidates) {
        ...
        deduped.set(key, scoreSupplementCandidate(item) > scoreSupplementCandidate(existing) ? item : existing);
    }
    return curateDerivedSupplementItems([...deduped.values()].sort((left, right) => scoreSupplementCandidate(right) - scoreSupplementCandidate(left)), text, vehicleApplicability);
}
...
function extractValuation(text) {
    ...
    return {
        ...
        ...(acvNumbers.length >= 2
            ...
            : {}),
        acvConfidence: acvNumbers.length >= 2
            ? "medium"
            : acvNumbers.length === 1
                ? "medium"
                : undefined,
        acvReasoning: sanitizeHumanText(acvBlock ?? "") ||
        ...
        acvMissingInputs,
        dvStatus: dvNumbers.length >= 2 || (dvExplicitRange && dvNumbers.length === 2)
            ? "estimated_range"
            : dvNumbers.length === 1
                ? "provided"
                : "not_determinable",
        ...
        ...(dvNumbers.length >= 2
            ...
            : {}),
        dvConfidence: dvNumbers.length >= 2
            ? "medium"
            : dvNumbers.length === 1
                ? "medium"
                : undefined,
        dvReasoning: sanitizeHumanText(dvBlock ?? "") ||
...
function looksLikeOperationSupportLine(line) {
    ...
    const mentionsGap = lower.includes("missing") ||
...
function deriveOperationTitles(line) {
    const lower = line.toLowerCase();
    const titles = [];
    ...
    if ((lower.includes("bumper") || lower.includes("lamp") || lower.includes("fender")) &&
...
function inferCategory(line) {
    const lower = line.toLowerCase();
    if (lower.includes("scan") || lower.includes("diagnostic"))
        return "scan";
...
function findValuationBlock(text, keywords) {
    const paragraphs = text
        .split(/\n\s*\n/)
        .map((part) => stripMarkdown(part).trim())
        .filter(Boolean);
    const paragraphMatch = paragraphs.find((paragraph) => keywords.some((keyword) => paragraph.toLowerCase().includes(keyword)));
    if (paragraphMatch)
...
function splitIntoReasoningUnits(text) {
    return text
        .split(/\n|;|\.(?=\s+[A-Z])/)
        .map((part) => sanitizeHumanText(stripMarkdown(part)))
        .filter((part) => part.length > 30);
}
function buildSupplementItemsFromReasoning(line) {
...
function sanitizeHumanText(value) {
    return value
        .replace(/^[^A-Za-z0-9$]+/, "")
        .replace(/^(?:what looks reasonable|what still needs support|what looks aggressive|what stands out|documented positives|likely remaining gaps|support posture|estimate position)\s*:\s*/i, "")
...
function extractReasoningBlocks(text) {
    const paragraphs = text
        .split(/\n\s*\n/)
        .map((part) => sanitizeHumanText(stripMarkdown(part)))
        .filter(Boolean);
...
function splitOperationalClauses(value) {
    return value
...
function scoreSupplementCandidate(item) {
    ...
    if (lower.includes("coolant") || lower.includes("bleed") || lower.includes("refill"))
        score += 90;
    if (lower.includes("corrosion") || lower.includes("cavity wax") || lower.includes("seam sealer"))
        score += 15;
    ...
    if (lower.includes("not documented") || lower.includes("not clearly") || lower.includes("underwritten"))
        score += 40;
    if (looksLikeMetaCommentary(item.rationale))
        score -= 400;
    score += Math.min(item.rationale.length, 120);
    return score;
}
function explicitlySignalsRange(value) {
    const lower = value.toLowerCase();
    return (lower.includes("range") ||
        lower.includes("between") ||
        lower.includes("low") && lower.includes("high"));
}
function inferSupplementKind(line) {
    const lower = line.toLowerCase();
...
function inferItemReason(title, line) {
    const lower = line.toLowerCase();
...
function inferFallbackTitles(line) {
    const lower = line.toLowerCase();
    const titles = [];
...
function curateDerivedSupplementItems(items, sourceText, vehicleApplicability) {
    if (items.length <= 1)
        return items;
    const lowerSource = sourceText.toLowerCase();
    const frontSpecificExists = items.some((item) => inferDerivedSupplementFamily(item.title) === "front_structure_scope" &&
        !isDerivedGenericFallback(item.title));
    const filtered = items.filter((item) => {
        if (!(0, vehicleApplicability_1.isVehicleContentApplicable)(`${item.title} ${item.rationale} ${item.evidence ?? ""} ${item.source}`, vehicleApplicability)) {
            return false;
        }
    ...
    for (const item of filtered) {
        ...
        kept.push({
            ...
            evidence: item.evidence
                ? (0, vehicleApplicability_1.sanitizeVehicleSpecificText)(item.evidence, vehicleApplicability) || undefined
                : undefined,
            source: (0, vehicleApplicability_1.sanitizeVehicleSpecificText)(item.source, vehicleApplicability) || item.source,
        });
...
function inferDerivedSupplementFamily(title) {
...
function isDerivedGenericFallback(title) {
    return [
...
function hasDerivedAlignmentEvidence(value) {
...
function hasDerivedMeasurementEvidence(value) {
    return (value.includes("measure") ||
...
function hasDerivedSupportScopeEvidence(value) {
...
function scoreNarrativeParagraph(value) {
    const lower = value.toLowerCase();
    ...
    if (lower.includes("support"))
        score += 20;
    if (looksLikeMetaCommentary(value))
        score -= 120;
    return score;
}
