...
exports.dedupeRepeatedDocumentSentences = dedupeRepeatedDocumentSentences;
exports.buildCarrierReport = buildCarrierReport;
exports.buildExecutiveSummary = buildExecutiveSummary;
exports.formatRepairIntelligenceSourceStatus = formatRepairIntelligenceSourceStatus;
const buildExportModel_1 = require("./buildExportModel");
const exportResearchSections_1 = require("./exportResearchSections");
...
const appraisalAwardEvaluator_1 = require("@/lib/ai/appraisalAwardEvaluator");
const narrativeNormalization_1 = require("@/lib/ai/narrativeNormalization");
/**
 ...
 */
function dedupeRepeatedDocumentSentences(document) {
    const seen = new Set();
    ...
    for (const row of document.summary) {
        if (row.value.trim().length >= 90)
            seen.add(normalize(row.value));
    }
    return {
        ...document,
...
function buildCarrierReport({ report, analysis, panel, assistantAnalysis, renderModel, workspaceData, exportResearchSnapshot, }) {
    ...
    const fallbackComparisons = analysis?.estimateComparisons ?? report?.analysis?.estimateComparisons;
    const structuredComparisons = (0, estimateComparisons_1.normalizeWorkspaceEstimateComparisons)(workspaceData ? (workspaceData.estimateComparisons ?? null) : fallbackComparisons);
    const dedupedComparisonRows = (0, estimateComparisonPresentation_1.dedupeEstimateComparisonRationales)(structuredComparisons.rows);
    const strongestDisputes = topItems.length > 0
    ...
    const canonicalVehicle = (0, buildExportModel_1.resolveCanonicalVehicleLabel)(exportModel) || "Unspecified";
    // `||`, not `??`: an empty-string VIN previously rendered the field BLANK
    // (empty string is falsy but not nullish).
    const canonicalVin = (0, buildExportModel_1.resolveCanonicalVin)(exportModel) || "Unspecified";
    const canonicalInsurer = (0, buildExportModel_1.resolveCanonicalInsurer)(exportModel);
    ...
    const document = {
        ...
        header: {
            ...
            subtitle: "Technical, procedural, evidentiary, and negotiation-aware repair position built from the current estimate, structured analysis, and supporting documentation.",
        ...
        sections: [
            ...(exportModel.oemContradictions.length > 0
                ? (0, exportResearchSections_1.buildExportResearchSections)(exportResearchSnapshot)
                : []),
            ...
            ...(dedupedComparisonRows.length > 0
                ? [{
                        ...
                        comparisonRows: dedupedComparisonRows.slice(0, 8).map((row) => ({
                            ...
                            delta: row.delta !== null && row.delta !== undefined && `${row.delta}`.trim() !== ""
                                ? `${row.delta}`
                                : undefined,
                            note: row.notes?.[0],
                        })),
...
function buildCaseUpdateBullets(report) {
...
function buildRetrievalSummaryBullets(summary) {
    return compact([
        ...
        ...summary.sourcesInfluencingFindings.slice(0, 6).map((source) => `${source.title} (${source.sourceType}) influenced ${source.relatedFindingIds.length} finding(s).`),
    ]);
}
function buildConfidenceIntegrityBullets(integrity) {
    return compact([
        ...
        (0, reviewCompleteness_1.buildReviewCompletenessMessage)({
        ...
        (0, reviewCompleteness_1.buildIndexedExclusionAuditNote)({
        ...
        integrity.missingCriticalEvidence.length > 0
            ...
            : undefined,
        ...integrity.confidencePenalties.map((penalty) => `${penalty.reason}: -${penalty.impact}. ${penalty.explanation}`),
        integrity.userFacingDisclosure,
    ]);
}
function buildAppraisalRecommendationBullets(exportModel, isComparison) {
    const award = (0, appraisalAwardEvaluator_1.evaluateAppraisalAward)({
    ...
    return compact([
        `Award posture: ${(0, appraisalAwardEvaluator_1.formatAppraisalAwardPosture)(award.posture)}.`,
 ...
 */
function dedupeListFragments(items) {
    const seen = new Set();
    const kept = [];
...
function buildFileReviewDiagnosticsBullets(exportModel) {
    const rows = (0, reviewCompleteness_1.buildFileReviewDiagnosticRows)({
...
function buildDisputeStrategyBullets(strategy) {
    return compact([
        ...
        strategy.hardFights.length > 0
            ...
            : undefined,
        ...strategy.recommendedSequence.slice(0, 5).map((item, index) => `${index + 1}. ${item}`),
    ]);
}
function formatCompletenessStatus(status) {
...
function buildExecutiveSummary(params) {
    ...
    const normalizeForContainment = (value) => (value ?? "").toLowerCase().replace(/[^a-z0-9]+/g, " ").trim();
    const disputesAlreadyStated = Boolean(params.strongestDisputes) &&
        normalizeForContainment(params.whyItWins).includes(normalizeForContainment(params.strongestDisputes));
    const sentences = [
...
function resolveCarrierExportModel(params) {
...
function buildTopDisputeDriverBullets(exportModel, fallbackItems) {
    ...
    return dedupeBullets(fallbackItems.slice(0, 6).map((item) => `${displayOperationLabel(item.title)}\n\n${item.rationale}${item.evidence ? ` Current support: ${item.evidence}` : ""}`));
}
function buildExplainabilityBullets(exportModel) {
...
function cleanExplainabilityDetail(value, title) {
...
function buildEvidenceBasisLabel(finding) {
    const raw = [
    ...
    const labels = [];
    if (/invoice|receipt|paid|vendor/.test(normalized))
        labels.push("invoice");
    if (/photo|image|visible/.test(normalized))
        labels.push("photo");
    if (/scan|dtc|diagnostic/.test(normalized))
        labels.push("scan");
    ...
    if (/oem|procedure|position statement|repair manual/.test(normalized))
        labels.push("OEM procedure reference");
    if (/comparison|shop estimate|carrier estimate|estimate/.test(normalized))
        labels.push("estimate comparison");
    if (/policy|coverage|endorsement/.test(normalized))
        labels.push("policy record");
    return labels.length > 0 ? joinHumanList([...new Set(labels)]) : "Current file evidence";
}
function formatExplainabilityStatus(value) {
    return formatRepairIntelligenceSourceStatus(value);
}
function formatRepairIntelligenceSourceStatus(value) {
    ...
    if (/inferred|referenced/i.test(value))
        return "Referenced but not produced";
    return formatSupportStatus(value);
}
function sanitizeReportProse(value) {
    return (0, narrativeNormalization_1.normalizeNarrativeProse)(value
...
function limitToReadableParagraph(value) {
    if (value.length <= 500)
...
function buildOemContradictionBullets(exportModel) {
...
function buildOemProcedureSupportBullets(exportModel, documentedStrengths) {
    ...
    const makeSpecificRetrieval = make
        ...
        : retrievalSupport;
    const noAuthorityRecord = make && makeSpecificRetrieval.length === 0
        ? [
...
function buildMissingVerificationBullets(exportModel) {
...
function buildRecommendedNextActionBullets(report, exportModel) {
...
function buildNegotiationLeverageBullets(exportModel) {
...
function buildSourceCitationSummary(report, analysis, exportModel) {
    const bullets = [
...
function formatComparisonLabel(category, operation, partName) {
    const cleanedOperation = displayOperationLabel(operation);
    const parts = [category, cleanedOperation, partName]
        .map((part) => part?.trim())
        .filter((part) => Boolean(part && !/^(repair operation|n\/a|none|unknown)$/i.test(part)));
...
function buildCredibilityConclusion(exportModel) {
...
function buildWhyItWins(exportModel, report, analysis) {
    ...
    if (topItems.length > 0) {
        ...
        return analysisMode === "comparison"
            ? `The current file most clearly supports review of ${joinHumanList(topItems.map((item) => displayOperationLabel(item.title).toLowerCase()))} against the competing repair documents.`
            : `The file most clearly leaves open ${joinHumanList(topItems.map((item) => displayOperationLabel(item.title).toLowerCase()))}.`;
    }
...
function buildValuationBullets(exportModel) {
    const bullets = [];
    ...
    bullets.push(`For a full valuation, continue at ${buildExportModel_1.COLLISION_ACADEMY_HANDOFF_URL}.`);
    return bullets;
}
function renderValuationBullet(label, params) {
    const parts = [];
    if (label === "Market Preview" &&
...
function buildSourceSummary(report, analysis, exportModel) {
...
function displayOperationLabel(value) {
    return (0, presentationText_1.normalizeEstimateOperationLabel)({ label: value, operation: value })
        || (0, presentationText_1.cleanOperationDisplayText)(value)
        || value
        || "Repair Operation";
}
function cleanCarrierReportNarrative(document) {
    return {
        ...document,
        header: {
...
function cleanUserFacingRepairProse(value) {
    return value
        .split(/\r?\n/)
        .filter((line) => {
...
function formatVehicleConfidence(exportModel) {
...
function cleanValuationReasoning(reasoning) {
    if (!reasoning)
        return null;
    const cleaned = reasoning.replace(/\s+/g, " ").trim();
    if (!cleaned)
        return null;
    if (/^(acv|dv|diminished value)\s+is\s+not determinable from the current documents\.?$/i.test(cleaned) ||
        /^not determinable from the current documents\.?$/i.test(cleaned)) {
        return null;
    }
    return trimTrailingPunctuation(cleaned);
}
function selectReportSupplementItems(items) {
    if (items.length <= 8) {
        return items;
    }
...
function toHumanReadableSourceLabel(source) {
    const trimmed = trimTrailingPunctuation(source).trim();
    if (!trimmed)
        return undefined;
    if (/repair-pipeline|pipeline evidence|assistant reasoning|structured narrative|structured analysis|supplement analysis|missing procedures|scan analysis|calibration analysis|drive knowledge base|retrieved evidence|^inline-\d+$|^retrieved-\d+$/i.test(trimmed)) {
        return undefined;
    }
    ...
    if (/^[a-z0-9_-]{12,}$/i.test(withoutOpaqueId))
        return undefined;
...
function cleanCarrierSummarySentence(value) {
    const cleaned = (value ?? "")
...
function normalizeCarrierConclusionConcept(value) {
    const normalized = value
        .toLowerCase()
        .replace(/[^a-z0-9\s]/g, " ")
        .replace(/\s+/g, " ")
        .trim();
    if (normalized.includes("credible") ||
    ...
    if (normalized.includes("remaining gaps") ||
