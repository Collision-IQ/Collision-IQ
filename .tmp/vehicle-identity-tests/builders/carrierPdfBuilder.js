...
function buildCarrierReport({ report, analysis, panel, assistantAnalysis, renderModel, workspaceData, }) {
    ...
    return {
        ...
        header: {
            ...
            subtitle: "Professional repair-position summary built from the current estimate, structured analysis, and supporting documentation.",
        ...
        sections: [
            ...
            },
            {
                ...
                bullets: compact([
                    ...
                    (0, buildExportModel_1.resolveCanonicalVin)(exportModel) ? `VIN: ${(0, buildExportModel_1.resolveCanonicalVin)(exportModel)}.` : undefined,
            ...
            ...(exportModel.findingReasoning.length > 0
                ? [{
                        ...
                        bullets: exportModel.findingReasoning.slice(0, 6).map((finding) => `${finding.priorityRank ?? ""}. ${finding.issue} | Why: ${finding.why_it_matters} | Proof: ${finding.what_proves_it} | Next: ${finding.next_action} | Evidence: ${capitalize(finding.evidenceLevel)}`),
                    }]
                : []),
                ...
                : []),
            {
                ...
                bullets: topItems.length > 0
                    ? topItems.map((item) => `${displayOperationLabel(item.title)}: ${item.rationale}${item.evidence ? ` Support noted: ${item.evidence}` : ""}`)
        ...
        footer: [
            ...
            `ACV and diminished value references are preliminary only. For a full valuation, continue at ${buildExportModel_1.COLLISION_ACADEMY_HANDOFF_URL}`,
        ],
    };
}
...
function displayOperationLabel(value) {
    return (0, presentationText_1.cleanOperationDisplayText)(value) || value || "Repair Operation";
}
