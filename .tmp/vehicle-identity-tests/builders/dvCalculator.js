...
function calculateDV(params) {
    const { repairCost = 0, structural, airbag, adas, hybrid, multiPanel, vehicleYear, isLuxury, } = params;
    const hasQualitativeExposure = Boolean(structural || airbag || adas || hybrid || multiPanel);
    if (!repairCost && !hasQualitativeExposure)
        return null;
    let percent = 0.05;
    if (repairCost > 5000)
        percent = 0.1;
    if (repairCost > 15000 || structural || airbag)
        percent = 0.15;
    if (repairCost > 0 && (adas || hybrid || multiPanel))
        percent = Math.max(percent, 0.08);
    if (isLuxury)
        percent += 0.03;
    let ageFactor = 1;
    if (vehicleYear) {
    ...
    if (repairCost <= 0 && hasQualitativeExposure) {
        return {
            low: 0,
            high: 0,
            confidence: adas || hybrid || multiPanel
                ? "low_to_moderate"
                : structural || airbag
                    ? "medium"
                    : "low",
            rationale: "The repair context suggests diminished value exposure may exist, but the current data does not support a quantified range yet.",
        };
    }
    return {
        low,
        high,
        confidence: structural || airbag
