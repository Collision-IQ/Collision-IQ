...
exports.generateNegotiationResponse = generateNegotiationResponse;
const supplementBuilder_1 = require("./supplementBuilder");
function generateNegotiationResponse(result) {
    ...
    const bullets = keyIssues
        .filter((finding) => supplements.some((item) => item.title.toLowerCase() === finding.title.toLowerCase() || finding.detail.toLowerCase().includes(item.title.toLowerCase())))
