...
exports.deriveStructuralApplicability = deriveStructuralApplicability;
exports.deriveStructuralApplicabilityFromResult = deriveStructuralApplicabilityFromResult;
exports.isStructuralSupplementSupported = isStructuralSupplementSupported;
exports.filterStructuralTitles = filterStructuralTitles;
function deriveStructuralApplicability(source) {
    const rawText = [
        source.rawText,
    ...
    const structuralMeasurementVerification = explicitMeasurement ||
        oemMeasurementSupport ||
        supportReplacementScope ||
        explicitPull ||
        explicitSetup ||
        strongVisualDistortion;
    const structuralSetupRequired = explicitSetup ||
        oemSetupSupport ||
        strongVisualDistortion;
    const pullOrRealignmentRequired = explicitPull ||
        oemPullSupport ||
        strongVisualDistortion;
    const structuralRepairOrSectioningRequired = explicitRepairOrSection ||
        oemRepairSupport;
    const guardedSetup = materialProfile === "aluminum_sensitive" && !explicitSetup && !oemSetupSupport && !strongVisualDistortion
        ? false
        : structuralSetupRequired;
    const guardedPull = materialProfile === "aluminum_sensitive" && !explicitPull && !oemPullSupport && !strongVisualDistortion
        ? false
        : pullOrRealignmentRequired;
    const guardedRepair = materialProfile === "aluminum_sensitive" && !explicitRepairOrSection && !oemRepairSupport
        ? false
...
function deriveStructuralApplicabilityFromResult(result) {
...
function filterStructuralTitles(items, applicability) {
    return items.filter((item) => isStructuralSupplementSupported(item.title, applicability));
}
function detectMaterialProfile(vehicle, rawText) {
    const make = (vehicle?.make || "").toLowerCase();
    const aluminumSensitiveMake = ["tesla", "rivian", "lucid", "audi", "jaguar", "land rover"].includes(make);
...
function hasAffirmativeSignal(text, tokens) {
...
function hasAnyList(values, tokens) {
    const text = (values ?? []).join("\n").toLowerCase();
    return hasAny(text, tokens);
}
function collectReasons(values) {
