...
exports.extractEstimateOps = extractEstimateOps;
exports.normalizeEstimateLineText = normalizeEstimateLineText;
...
function parseEstimate(text) {
    ...
    for (const line of lines) {
        const opMatch = line.match(/^#?\s*(\d+)?\s*(R&I|Repl|Rpr|Blnd|Subl|Algn|Proc)\s+(.*)$/i);
        if (opMatch) {
            out.lines.push({
...
function normalizeEstimateLineText(value) {
    return value
...
function extractTrailingLaborHours(value) {
...
function stripTrailingLaborHours(value) {
    const normalized = normalizeEstimateLineText(value);
    const match = normalized.match(/^(.*?)(?:\s+(\d{1,2}(?:\.\d+)?))$/);
    if (!match)
