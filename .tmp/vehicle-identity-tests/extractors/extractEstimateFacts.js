...
function extractInsurer(text) {
    const knownFromText = COMMON_INSURERS.find((carrier) => new RegExp(`\\b${escapeRegExp(carrier)}\\b`, "i").test(text));
    const labeled = text.match(/\b(?:insurer|insurance company|carrier|insurance co(?:mpany)?)\b\s*[:#-]\s*([A-Za-z][A-Za-z .&'-]{1,40})/i)?.[1]?.trim();
    return resolveCanonicalInsurerCandidate({ value: labeled, source: "labeled" }, { value: knownFromText, source: "known_carrier" });
}
...
function resolveCanonicalInsurerCandidate(...candidates) {
