...
(async () => {
  ...
  const record = { generatedAt: new Date().toISOString(), sourceHash: normalized.sourceHash, naturalWidth: normalized.naturalWidth, naturalHeight: normalized.naturalHeight, falEndpoint: DAMAGE_SEGMENTATION_MODEL, requestId: segmented.requestId, cached: segmented.cached, promptVersion: "visible-damage-v3", validationVersion: MASK_VALIDATION_VERSION, localizationBoxes: analysis.zones.map((z) => z.boundingBox), returnedScores: segmented.scores, returnedBoxes: segmented.returnedBoxes, acceptedMasks: masks, rejectedMasks: segmented.rejected };
