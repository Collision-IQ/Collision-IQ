...
function buildSystemPrompt(annotationStyle: AnnotationStyle): string {
  ...
  return [
    ...
    "- For surface damage (scratches, scuffs, dents, creases), provide 3 to 8 positivePoints spaced ALONG the visible damage itself — trace the scratch or scuff path across the panel. These points directly drive where heat is drawn, so every point must sit on marked or visibly damaged paint, never on clean paint, chrome trim, or a gap between damage.",
    "- Choose damageType precisely: use 'scuffed' for surface scratches/scuffs/abrasions and 'deformed' for dents, creases, and buckling; reserve 'broken', 'torn', 'cracked', and 'displaced' for a component that is itself visibly fractured, torn, or out of position, and 'missing' when a component is absent.",
    ...
    "- When a zone sits at or near a wheel opening, you MUST place a negativePoint on the wheel rim and one on the tire, and the boundingBox yMax must stay above the tire's road contact — never include pavement, gravel, or shadow under the vehicle in any box.",
    "- segmentationPrompt must describe the damaged SURFACE, not a whole component: say 'scratched rear door outer panel skin' or 'scuffed quarter panel above wheel arch', never 'wheel', 'tire', 'rear wheel area', 'rocker area', or any prompt a segmenter could satisfy by selecting an entire round component or the ground. Never use generic prompts such as vehicle, collision, car damage, damaged car, or wheel.",
