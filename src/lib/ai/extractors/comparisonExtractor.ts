...
export function extractComparisonFacts(
  ...
  const fact = (parsed: ParsedEstimate) => ({
    ...
    calibrationTransport: hasLine(
      parsed,
      /transport vehicle to sublet|transport vehicle from sublet|transport.*alignment|transport.*calibration/i
    ),
    ...
    clampZoneRepair: hasLine(
      parsed,
      /clamp zone|pinch weld repair|clamping area/i
    ),
    ...
    threeStageRefinish: hasLine(
      parsed,
      /three-?stage|3-?stage|tri-?coat|three stage/i
    ),
