...
function buildIntegrationInventory(env: NodeJS.ProcessEnv): IntegrationInventoryItem[] {
  return [
    inventoryItem(env, "OpenAI / GPT-5.5", ["src/lib/openai.ts", "src/lib/modelConfig.ts", "src/app/api/chat/route.ts", "src/app/api/analysis/route.ts"], ["OPENAI_API_KEY", "COLLISION_IQ_MODEL_PRIMARY", "COLLISION_IQ_MODEL_HELPER"], true, "/api/admin/integrations-health", ["web", "mobile", "reports", "chat"]),
    inventoryItem(env, "Anthropic / Claude", ["src/lib/modelConfig.ts", "src/lib/ai/providerTextGeneration.ts"], ["ANTHROPIC_API_KEY", "ANTHROPIC_MODEL_PRIMARY", "ANTHROPIC_BASE_URL"], false, "/api/admin/integrations-health", ["chat", "reports"]),
    ...
    inventoryItem(env, "Agents / internal routing", ["src/lib/ai/agentRetrievalTrace.ts", "src/app/api/internal/agent-review/route.ts"], ["OPENAI_API_KEY", "COLLISION_IQ_MODEL_HELPER"], true, "/api/admin/integrations-health", ["reports", "chat", "admin"]),
