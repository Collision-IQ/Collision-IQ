...
run("bottom report workspace restores interactive in-context report review", () => {
  ...
  assert.match(pageSource, /flex h-\[100svh\] flex-col overflow-hidden/);
  assert.doesNotMatch(pageSource, /flex min-h-\[100svh\] flex-col overflow-x-hidden/);
  assert.match(pageSource, /data-report-bottom-viewer/);
...
run("Chat workspace stays compact without reports and expands when report remains below", () => {
  ...
  assert.match(shellSource, /ci-workstation flex min-h-0 flex-1/);
  ...
  assert.match(shellSource, /relative z-10 flex flex-1 min-h-0 w-full max-w-none flex-col/);
  assert.doesNotMatch(shellSource, /contentWrapClass = bottom \? "flex-1" : "shrink-0"/);
  assert.doesNotMatch(shellSource, /reviewRowHeightClass = bottom/);
  assert.doesNotMatch(shellSource, /lg:h-\[min\(760px,calc\(100svh-96px\)\)\]/);
  assert.match(shellSource, /lg:min-h-\[520px\]/);
  assert.match(shellSource, /reviewRowHeightClass = "lg:h-full lg:min-h-\[520px\]"/);
  assert.match(shellSource, /chatPanelHeightClass = "h-full min-h-0"/);
  assert.match(shellSource, /rightRailHeightClass = "h-full min-h-0"/);
  assert.doesNotMatch(shellSource, /lg:min-h-\[420px\]/);
  assert.doesNotMatch(shellSource, /sticky top-3 hidden min-h-0/);
  assert.match(shellSource, /grid flex-1 min-h-0[\s\S]{0,120}overflow-hidden/);
  assert.match(shellSource, /hidden h-full min-h-0 w-full flex-col overflow-hidden/);
  assert.match(shellSource, /flex-1 min-h-0 overflow-y-auto p-3/);
  assert.match(shellSource, /\{bottom \? <div className="mt-2 shrink-0 sm:mt-3">\{bottom\}<\/div> : null\}/);
  ...
  assert.match(pageSource, /<div className="flex h-\[100svh\] flex-col overflow-hidden bg-background text-foreground">/);
  ...
  assert.match(pageSource, /: "relative flex h-full min-h-0 w-full flex-col"/);
  ...
  assert.match(pageSource, /<footer className="mt-auto border-t border-border bg-card\/80 px-4 py-4 text-card-foreground sm:py-8"/);
