...
run("Chat workspace stays compact without reports and expands when report remains below", () => {
  ...
  assert.match(shellSource, /relative z-10 flex min-h-0 w-full max-w-none flex-1 flex-col/);
  ...
  assert.match(shellSource, /: "lg:h-full lg:min-h-\[520px\]"/);
  assert.match(shellSource, /chatPanelHeightClass = "lg:h-full"/);
  assert.match(shellSource, /rightRailHeightClass = "lg:h-full"/);
