/**
 * The delta lane's second document as the pipeline actually produces it: the
 * Forensic Estimate Analysis, alone. The consolidation to two documents means
 * the per-finding card dump no longer ships — the annotated estimate is the
 * record, the forensic report is the adjudication — so this file asserts the
 * forensic front matter, the claim-identity header (read from the document and
 * redacted to export policy), continuous page numbering, and that the OEM
 * report keeps its plain cover so the two reports cannot quietly converge.
 */
...
describe("delta findings report", () => {
  it("ships the forensic estimate analysis as the only findings document", async () => {
    ...
    // The consolidation is exactly two documents per review: the annotated
    // estimate is the record, this report is the adjudication. The legacy
    // per-finding card dump must not ride along behind the forensic report.
    expect(all).not.toMatch(/Finding number:/);
  });

  it("numbers its pages as one continuous document", async () => {
    const result = await buildDeltaReport();
    const pages = await extractPageTexts(result.findingsReportBytes!);
    expect(pages.length).toBeGreaterThan(1);
    pages.forEach((page, index) => {
      expect(page).toMatch(new RegExp(`Page ${index + 1}\\b`));
    });
