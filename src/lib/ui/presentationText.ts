...
export function cleanPresentationText(value: string | null | undefined): string {
  ...
  return value
    ...
    .replace(/[Â·]+/g, "·")
    .replace(/\b((?:rt|lt)\/rear\s+r\s*&\s*i\s+wheel)\s*\.?\s*2\s*m\b/gi, "$1")
    .replace(/\bwheel\s*\.?\s*2\s*m\b/gi, "wheel")
    .replace(/\bbattery\s*\.?\s*3\s*e\b/gi, "battery")
    .replace(/\bwindshield\s+tesla\s*0\.\s*5\b/gi, "windshield Tesla")
    .replace(/\btesla\s*0\.\s*5\b/gi, "Tesla")
    .replace(/\b(bumper\s+cover)\s*\d+(?:\.\d+)?\b/gi, "$1")
    .replace(/\b(scan)\s+\+\d+%\s*\d+(?:\.\d+)?\s*t\s*m\b/gi, "$1")
    .replace(/\b(scan)\s+\+\d+%\s*\d+(?:\.\d+)?t\b/gi, "$1")
    .replace(/\bfailure\.\s*Procedure cost open to invoice\b/gi, "repair support remains open pending invoice documentation")
...
export function cleanUserFacingPresentationText(
  ...
  cleaned = cleaned
    ...
    .replace(/\bCCC\s+confirms\s+this\s+operation\s+is\s+required\.?/gi, "CCC Secure Share source confirms this estimate line was present in the structured estimate data.")
    ...
    .replace(/\b(scan)\s+\+\d+%\s*\d+(?:\.\d+)?t\b/gi, "$1")
    .replace(/\bfailure\.\s*Procedure cost open to invoice\b/gi, "repair support remains open pending invoice documentation")
    .replace(/\bsome of the repair steps are still only partly\b\.?/gi, "some of the repair steps are still only partly verified.")
    .replace(/\bif calibration, alignment, or hidden mounting issues were not fully\b\.?/gi, "if calibration, alignment, or hidden mounting issues were not fully verified.")
    ...
    .replace(/\bmake sure the claim handling stays\b\.?/gi, "make sure the claim handling stays clear and documented.")
    ...
    .replace(/\bAsk the (?:shop|appraiser) to (?:submit|provide) the (?:applicable\s+)?(?:OEM|OE|GM|manufacturer)\s+(?:procedure|procedures|position statement)(?: before (?:I|we|the bot|Collision IQ) can reason further)?\.?/gi, "Collision IQ identified this as an OEM-procedure-dependent issue and should retrieve and apply the applicable authority before assigning the remaining documentation gap.")
    .replace(/\b(?:Have|Tell) the (?:shop|appraiser) (?:submit|provide) (?:the\s+)?(?:OEM|OE|GM|manufacturer)\s+(?:procedure|procedures|position statement)\b/gi, "Collision IQ should retrieve and apply the applicable OEM authority first")
    .replace(/\bGive me the (?:OEM|OE|GM|manufacturer)?\s*(?:procedure|procedures|position statement) and (?:I|we)('ll| will) tell you\b/gi, "Collision IQ should identify the estimate line, search the authority sources, and explain what was found")
    .replace(/\bnot proven\b/gi, "not yet supported by a named authority or proof source")
